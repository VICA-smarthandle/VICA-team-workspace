...
스마트 핸들 터치센서 상태에 따라 다시 두 모드로 나뉜다. 아래 모드 정의와 전환 흐름은
현재 코드에 없는 구현 목표이며, 정지 유예·정지 지연 시간값과 STOP/ESTOP 등급은 팀
위험성 평가로 확정한다.
...
Nav2 자율주행 모드
├── 스마트 핸들 활성 모드
│   └── 터치센서가 일정 시간(예: 3초) 지속 감지되어 활성화된 상태.
│       가변저항(knob) 연속값을 CAN으로 motor에 전달해 보행 속도를 추종한다.
└── 스마트 핸들 비활성 모드
    └── 터치센서 미감지 상태. knob 값을 읽지 않고(또는 기존 디폴트값 고정)
        순수 Nav2 자율주행 속도로 주행한다.
```

터치센서와 가변저항(knob)은 별개 입력이다. 터치센서는 모드 활성/비활성만 판정하고,
가변저항은 활성 모드에서만 속도 보정에 사용한다. knob 게이팅(모드에 따른 knob 반영
여부)과 모드 전환 판정을 담당하는 노드는 아직 미정 `[TARGET]`이다.

### 2-1.1 모드 진입 안내 [구현 목표]

스마트 핸들 활성 모드 진입은 LLM 또는 지정된 멘트로 음성 안내한다. 예시 멘트:
...
스마트 핸들 활성 모드에서 터치센서가 미감지로 바뀌면 다음 순서로 처리한다. 이 흐름은
안전 정지 규칙이므로 유예 시간과 정지 지연 값, 최종 등급은 팀 확정이 필요하다.

```text
터치 미감지 시작
    → 짧은 유예(예: 1~2초, 잠깐 놓쳤다 다시 잡는 경우 대비)
    → 유예 초과 시 TTS 안내
