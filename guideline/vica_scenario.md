...
**knob 게이팅 필요 여부는 리코일 중립값 실측으로 결정한다** `[미검증]`. motor node에서
knob는 속도 상한이며 `allowed = max_linear_mps(1.0) × knob_pct/100`으로 적용된다. 가변저항
0~10 kΩ이 0~100 %로 환산된다. Nav2의 `max_velocity`가 0.26 m/s이므로 **knob 26 %(2.6 kΩ)
이상에서는 Nav2 속도에 영향이 없다.** 핸들 미접촉 상태의 값이 이 기준 이상이면 모드별 게이팅
없이 두 모드가 그대로 동작한다. 미만이면 비활성 모드에 하한을 적용해야 한다.

F1 프레임 미수신은 CAN 통신 이상이므로 모드와 무관하게 항상 정지 근거로 유지한다. 게이팅이
필요해지더라도 무시하는 대상은 knob **값**이며 프레임 **수신 여부**가 아니다.
...
| 항목 | 현재 | 영향 |
| --- | --- | --- |
| **리코일 중립 knob 값** | **`[미검증]`** | **2.6 kΩ(26 %) 이상이면 knob 게이팅이 불필요하다** |
| 비활성 모드 knob 하한 | 미정 | 위 실측이 2.6 kΩ 미만일 때만 필요 |
