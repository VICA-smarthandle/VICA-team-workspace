...
**구현은 `ESTOP`을 등급 축에서 뺐다.** 초안의 6단 등급은 "얼마나 나쁜가"와 "어떤
모드인가"를 한 축에 섞는다. E-stop은 STOP보다 심각한 것이 아니라 종류가 다르다 —
래치가 걸리고 관리자 reset이 있어야 풀리며 `emergency_stop_node`가 소유한다.

`RobotFault.severity`는 `OK`~`STOP`, `FAULT` 5단이고, 비상정지는 `RobotFault.latched`와
`RobotHealth.state == ESTOPPED`가 나타낸다. 아래 9.1절 표의 `ESTOP` 행은 **등급이 아니라
래치 대상**으로 읽는다. 정본은 `vica_architecture.md` 4.1절이다.

이 판정은 실기동 관측에서 나왔다. 등급으로 폭주 억제를 풀면 모터 진단이 수신되지
않을 뿐인 결함이 초당 한 건씩 알림을 냈다(occurrence_count 223회).
