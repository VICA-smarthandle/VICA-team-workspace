...
상태: **Phase 1~5 완료 / 실기 검증 완료 (2026-07-29)** — 진행 상태는 10절.
      회전 임계값 25°는 실주행으로 확정됐다(10.1e절). 남은 것은 `dev` 머지뿐이다.
...
turn_guide_node:
  ros__parameters:
    odom_topic: /odom
    window_sec: 1.5
    enter_threshold_deg: 25.0    # 2026-07-29 실주행으로 확정 (10.1e절)
...
> **5a·5b 모두 2026-07-29에 완료했다.** 결과는 10.1d절·10.1e절. 임계값 25°는
> 실주행 8회 판정에서 전부 25.0~26.6°에 걸렸고 직진 오탐이 0회여서 확정됐다.

> 2026-07-29 실기 시리얼 검증에서 `/odom`은 스크립트가 만든 **잡음 0, 정확히
> 30°/s**인 합성 데이터였다. 실제 EKF yaw는 드리프트와 잡음이 있고 회전 속도도
> 일정하지 않다. 임계값이 그 시점까지 `[미검증]`이던 이유가 이것이다.
> **같은 날 5b 실주행으로 확정됐다(10.1e절): 실제 회전 8회가 전부 25.0~26.6°에
> 걸렸고 40초 연속 직진에서 오탐 0회.**

**정지 상태 yaw 드리프트 측정은 2026-07-29에 완료했다(10.1c절).** 300초 창 누적
최대 0.060°, 임계값까지 여유 417배, 오탐 0회. `ekf.yaml`을 손볼 이유가 없으므로
5b는 EKF 재설정 없이 진행한다. AGENTS.md 6장의 D455 IMU 융합 `[미검증]` 표기는
정지 상태에 한해 이 측정으로 근거가 생겼고, **주행 중 융합 품질은 여전히
`[미검증]`이다.**
...
### 9.2 udev 규칙 — **노트북·Jetson 양쪽 설치·검증 완료 (2026-07-29)**

> **장비마다 따로 설치해야 한다.** 규칙은 처음 개발 노트북에만 설치했고, Jetson에는
> 없어서 Phase 5a 초반을 `/dev/ttyUSB0` 임시 파라미터 파일로 진행했다(10.1d 발견 1).
> **같은 날 Jetson에도 설치·검증을 마쳤다** — `/dev/vica_smart_handle -> ttyUSB0`,
> `ID_SERIAL_SHORT=B003UMKG`, `ID_MM_DEVICE_IGNORE=1` 확인. 저장소 기본 config
> 그대로 `connected: true`, `fault_code: 0`, `write_error_count: 0`으로 재연결했다.
...
### 10.1c 정지 상태 yaw 드리프트 측정 완료 (2026-07-29)

Jetson 실기에서 300초 측정했다. **판정: 안전 — 오탐 0회, 임계값까지 여유 417배.**

| 항목 | 값 |
| --- | --- |
| 측정 시간 / 샘플 | 300.0초 / 9005개 (30.0 Hz) |
| 총 드리프트 | −0.466° (−0.093°/분) |
| yaw 최소/최대 | −1.130° / +0.000° |
| **창 누적 \|Δyaw\| 최대** | **0.060°** (진입 임계값 25.0°, 여유 417.4배) |
| 정지 상태 오탐 | LEFT 0회, RIGHT 0회 |
| 샘플간 \|Δyaw\| | 최대 0.0088° / 평균 0.0018° |

`TurnDetector`가 실제로 비교하는 값은 1.5초 창의 누적 \|Δyaw\|다. 그 값이 임계값의
0.24%에 그쳤다.

**측정 구성 — 실제 운영보다 불리한 조건이다.**

| 항목 | 이번 측정 | 운영 |
| --- | --- | --- |
| EKF 입력 | `/imu/base_link`만 | `/imu/base_link` + `/wheel/odom` |
| 기동 노드 | rsp, realsense(Docker), imu adapter, ekf | + motor, Nav2, nvblox 등 |

`wheel_ekf.launch.py`를 `start_encoder:=false`로 띄웠다. 정지 상태에서 휠 오도메트리는
vyaw=0을 보고해 IMU 드리프트를 **억제**하므로, 그 입력을 뺀 이번 값은 운영 조건의
상한이다. `/cmd_vel*` 발행자를 하나도 띄우지 않았고 CAN에 쓰지 않았다.

> 휠을 포함하려면 C5 위치 피드백 방송이 필요하다. 측정 시점 `candump can1`에는
> `F1`(IO monitor)만 있고 C5가 없었다. `encoder_feedback`은 read-only이므로
> (`request_position_feedback: False`) motor node나 요청 모드가 있어야 `/wheel/odom`이
> 나온다. 그 경로는 CAN에 `PID_PNT_VEL_CMD(0xCF)`를 쓰므로 AGENTS.md 5장 범위다.

**드리프트는 발산하지 않는다.** 60초 구간별로 보면 180초에서 방향이 반전된다.

| 구간(초) | 시작 | 끝 | 구간 변화 | 구간 내 폭 |
| --- | --- | --- | --- | --- |
| 0–60 | 0.000 | −0.613 | −0.613 | 0.631 |
| 60–120 | −0.615 | −0.975 | −0.360 | 0.367 |
| 120–180 | −0.979 | −1.065 | −0.085 | 0.154 |
| 180–240 | −1.064 | −0.810 | **+0.255** | 0.283 |
| 240–300 | −0.809 | −0.464 | **+0.345** | 0.373 |

총 드리프트(−0.466°)보다 최저점(−1.130°)이 낮다. 시간에 비례하는 누적이 아니라
저주파 진동이므로 장시간 안내 주행에서 임계값에 접근하지 않는다.

**결론:** EKF 설정을 손볼 이유가 없다. Phase 5절의 "5b 이전 EKF 재검토" 조건은
해소됐다. 임계값 25°는 정지 상태 기준으로 과도하게 보수적이지도, 부족하지도 않다.
다만 **실주행 진동은 이 측정에 포함되지 않았다** — 임계값 확정은 여전히 5b다.

### 10.1d Phase 5a 완료 (2026-07-29, 바퀴 띄움)

**전 항목 통과.** 바퀴를 띄운 상태에서 사용자가 차체를 손으로 돌려 실제 IMU 회전을
만들었다. `/cmd_vel_req` 발행자(Nav2·Mission)는 띄우지 않아 주행 명령은 0이었다.

| 항목 | 결과 |
| --- | --- |
| 시리얼 배선·진단 | `connected: true`, `fault_code: 0`, `write_error_count: 0` |
| 서보·LED 진단 | `servo_ok`·`left_led_ok`·`right_led_ok` 모두 true (`haptic_ok: false`는 미부착) |
| 실제 IMU 회전 감지 | 좌우 7회 교대, 방향 오판 0회 |
| 히스테리시스 | 진입 25.36~29.54°(임계 25°), 이탈 4.77~9.57°(임계 10°) |
| `sequence_id` | 회전 진입마다 8→15로 정확히 1씩 증가 |
| 중앙 래치 fail-safe | motor node 부재 시 `motor_can_stale`로 자동 래치 |
| **원인 해제 후 자동 해제 방지** | motor 기동으로 원인이 사라져도 `ESTOP_RELEASED_WAIT_RESET` 유지 |
| 명시적 reset 수락 | `[RESET ACCEPTED] → READY_TO_GO` |
| **E-stop 우선순위** | **판정 7회 전부 무시, 핸들 ESTOP 유지** |
| 시리얼 단절 워치독 | 1500ms 후 `LINK_LOST` 빨간불, 재연결 시 복구 |

**E-stop 우선순위 — 두 계층 분리가 실기에서 확인됐다.**

```
 0.26  핸들 → ESTOP
62.79  turn_guide RIGHT  창누적 -25.85°  seq=8     ┐
63.89  turn_guide LEFT   창누적 +25.36°  seq=9     │ 판정은 7회 계속됐으나
  …                                                │ 핸들 상태는 0.26초 이후
74.09  turn_guide LEFT   창누적 +28.48°  seq=15    ┘ 단 한 번도 바뀌지 않았다
```

`turn_guide_node`(판단)는 E-stop과 무관하게 계속 판정하고,
`user_guidance_driver_node`(병합)가 E-stop을 최우선으로 두어 cue를 전부 눌렀다.
5.1절 우선순위 병합이 설계대로 동작한다.

**워치독은 소등이 아니라 빨간불이다.** 펌웨어 `STATE_LINK_LOST`(4)가 `setBoth(RED)`를
호출한다. 소등으로 오인하면 "펌웨어가 죽었다"와 구분하지 못한다.

#### 발견 1 — udev 규칙이 Jetson에 미설치였다 → **같은 날 해소**

9.2절의 "설치·검증 완료"는 **개발 노트북 기준**이었다. Jetson에는 규칙이 없어
`/dev/vica_smart_handle`이 존재하지 않았고, 시험 초반은 `serial_port`를
`/dev/ttyUSB0`으로 바꾼 임시 파라미터 파일로 진행했다(저장소 파일은 수정하지 않았다).

이 Jetson에는 CP2102 장치가 이미 `/dev/ttyUSB1`에 붙어 있어 열거 순서가 바뀌면
엉뚱한 포트를 연다. **2026-07-29 Jetson에 규칙을 설치해 해소했다**(9.2절).
저장소 기본 config로 재연결해 `connected: true`, `fault_code: 0`,
`write_error_count: 0`을 확인했다.

> **교훈: 장비 의존 설정은 저장소에 커밋해도 자동 적용되지 않는다.** udev·그룹 권한·
> 커널 모듈처럼 OS에 설치해야 하는 항목은 "완료"를 적을 때 **어느 장비인지** 함께
> 적는다. 노트북↔Jetson 이원 개발에서는 이 구분이 없으면 실주행 직전에 걸린다.

#### 발견 2 — 자극 조건: 앞바퀴를 완전히 든 상태였다

기록된 창 누적 **25~29°** 는 **앞바퀴를 완전히 들어올린 상태에서 차체를 좌우로
흔들어** 만든 값이다. 사용자 확인(2026-07-29): **바퀴가 접지된 상태에서 그냥 밀면
좌우로 움직이지 않는다.**

따라서 이 값을 "승객이 핸들을 당길 때의 외력"으로 읽으면 안 된다. 구동륜과 캐스터가
접지해 있으면 그 정도 요각이 나오지 않는다. 같은 이유로 앞서 단발 관측된 17.11°도
로봇을 직접 건드린 상황의 값으로 본다.

**Phase 5b에서 볼 것은 승객 외력이 아니라 주행 진동이다.** 노면 요철·모터 진동·
가감속에서 창 누적이 얼마까지 오르는지가 임계값 확정의 근거다.

> 자극 조건을 함께 적지 않으면 같은 숫자가 전혀 다른 결론으로 읽힌다. 10.1c의
> 정지 상태 417배(무접촉)와 이번 25~29°(차체를 들고 흔듦)는 애초에 다른 시험이다.

### 10.1e Phase 5b 완료 (2026-07-29, 바닥 주행)

**Smart Handle 안내 계층은 전 항목 통과했다.** 회전 임계값 25°가 실주행에서 확정됐다.

#### 1차 주행 — 완주

`현위치 → 방2 → 화장실 → 방2 → 안내소`, 총 211초, **갇힘·복구동작·실패 0회.**
...
**회전 판정 8회가 모두 핸들에 전달됐고 누락이 없다.**

| 판정 시각 | 방향 | 창 누적 | 핸들 반영 지연 |
| --- | --- | --- | --- |
| 58.78 | RIGHT | −25.29° | 0.44초 |
| 68.38 | RIGHT | −25.00° | 0.34초 |
| 71.73 | RIGHT | −25.65° | 0.49초 |
| 111.04 | RIGHT | −25.28° | 0.18초 |
| 127.68 | LEFT | +25.62° | 0.04초 |
| 145.53 | RIGHT | −25.46° | 0.19초 |
| 155.74 | RIGHT | −26.59° | 0.48초 |
| 205.64 | RIGHT | −25.44° | 0.08초 |

- 지연 0.04~0.49초(평균 0.28초). 10 Hz 전송·20 Hz 판정 주기의 예상 범위다.
- 이탈은 전부 −9.4~−9.98°. 진입 25°/이탈 10° 히스테리시스가 실주행에서 채터링 없이 동작.
- **직진 오탐 0회.** 방2→안내소 구간은 `v=0.26 m/s`로 40초 연속 직진했고 그동안
  핸들이 NORMAL을 유지했다.
- 도착 표시 **4회 모두 4.000초** 유지(`goal_succeeded` → ARRIVED → NORMAL).
- 비상정지: 물리 E-stop → **0.33초** 만에 핸들 ESTOP 전환.
- 211초 내내 `connected=true`, `fault_code=0`, `write_error_count=0`.

> **상태코드 5는 ARRIVED다.** 4가 LINK_LOST다(`protocol.py`·펌웨어 `#define`).
> 기록 도구에서 이 둘을 바꿔 쓰면 정상 도착이 FAULT로 보인다 — 실제로 겪었다.

#### 2차 주행 — 2구간 성공 후 안내소에서 갇힘

`안내소 → 화장실(64.8초) → 방2(41.9초) → 안내소(실패)`.
회전 판정 4회, 지연 0.13~0.23초, 도착 표시 2회 모두 4.000초로 **1차와 재현성이 있다.**

마지막 구간은 방2에서 1.83 m 진행한 (+1.72, +1.44)에서 `No valid trajectories out of
419!`로 굳었다. Spin 복구 3회도 실패했다.

| 확인 | 값 |
| --- | --- |
| 정적 지도 기준 footprint 충돌 | **0 / 793 표본** (지도상 통과 가능) |
| LiDAR 전방(±30°) 최근접 | 3.24 m (앞은 비어 있음) |
| LiDAR 전체 최근접 | **0.38 m @ −179°** (바로 뒤) |
| 뒤 장애물 (base_link 기준, LiDAR x=+0.185 보정) | 0.565 m |
| footprint 후방 한계 (−0.60 − padding 0.03) | **0.63 m** ← 장애물이 안에 들어옴 |
| 실제 차체 후방 (`base_link.stl` 실측) | 0.505 m ← 실제로는 안 닿음 |

**이것은 새 결함이 아니라 이미 규명된 planner-controller 불일치의 재현이다.**
`SmacPlanner2D`는 중심 셀만 보는 점 로봇 planner라 내접반경 0.277 m만 보장하는데
VICA는 외접반경 0.707 m다. planner가 통과 가능으로 그린 자리에서 방향을 틀면 후방이
장애물을 쓸고 DWB `ObstacleFootprint`가 궤적을 전부 거부한다. 상세는
`devlog/2026-07-29.md`. **Smart Handle 안내 계층의 결함이 아니므로 Phase 5b 판정에
영향을 주지 않는다.**

> 위 수치는 그 메커니즘의 구체적 실측 사례로서 값이 있다. 후방 footprint가 실측보다
> 9.5 cm 큰 것(안전측 여유)이 "실제로는 닿지 않는데 굳는" 상황을 만들 수 있음을 보인다.
> 다만 그 여유를 줄일지는 Smart Handle과 무관한 Nav2 안전 결정이므로 여기서 정하지 않는다.

#### 발견 — 브랜치가 `dev`보다 뒤처져 실패를 재현했다 `[해소됨]`

첫 주행 시도는 50초간 제자리 회전·후진만 반복하다 `goal_failed`했다. 원인은
`feat/user-guidance`가 `dev`의 Nav2 수정을 받지 않은 것이었다.

| | 머지 전 | 머지 후(`dev`) |
| --- | --- | --- |
| footprint 전방 | +0.15 m | **+0.305 m** |
| footprint 좌우 | ±0.1875 m | **±0.227 m** |
| planner | `GridBased`(구) | `nav2_smac_planner/SmacPlanner2D` |
| nvblox `esdf_slice_min_height` | 구 설정 | 0.05 |

전방 15.5 cm·좌우 4 cm를 실제보다 작게 인식하는 상태였다. **2026-07-27 전방 범퍼
충돌로 고친 값이 이 브랜치에 없었다.** `dev`를 머지하고 재빌드하니 곧바로 완주했다.

> **교훈: 기능 브랜치에서 실주행하기 전에 `dev` 머지 여부를 먼저 확인한다.**
> Smart Handle만 시험한다고 생각해도 실주행은 Nav2 전체를 쓴다. `git log
> feat/xxx..dev`가 비어 있는지 보는 것으로 끝난다.

#### 남은 관찰

- `write_error_count`가 1차 종료 후 **1**로 올라갔다(주행 중 0 유지, 노드 재기동
  구간에서 증가). 재현 조건 미확인 `[미검증]`.
- Mission Manager 서비스 응답이 **11.3초** 걸렸다. 콜백 안에서 Nav2 goal 전송까지
  하기 때문으로 보인다. 클라이언트 타임아웃을 15초로 잡으면 goal은 수락됐는데
  클라이언트만 죽는다 — 첫 시도에서 실제로 겪었다.
...
1. **`dev` 머지**: Phase 5 통과 조건이 충족됐다(10.2절). `feat/user-guidance`에는
   이미 `dev`가 머지되어 있으므로 반대 방향 머지만 남았다.

> 완료: ~~bench 8번~~, ~~실기 시리얼 검증~~, ~~udev 규칙 설치·전환(노트북·Jetson)~~,
> ~~`/odom` yaw 드리프트 측정~~, ~~Phase 5a(HIL, 바퀴 띄움)~~, ~~Phase 5b(바닥 주행)~~
> (2026-07-29, 9.2절·10.1b~10.1e절).

**Phase 5가 전부 끝났다. 회전 임계값 25°는 더 이상 `[미검증]`이 아니다.**

Smart Handle과 무관하게 남은 Nav2 과제(별건, `devlog/2026-07-29.md`):
planner-controller footprint 불일치 → `SmacPlannerHybrid` 검토. 2차 주행의 안내소
구간 실패가 이것이며 안내 계층 결함이 아니다(10.1e절).

선택 항목(필수 아님): 휠 오도메트리를 포함한 yaw 재측정. 10.1c는 IMU 단독이라
운영 조건의 상한이고 여유가 417배였으므로, 재측정해도 판정이 바뀌지 않는다.
Phase 5a에서 motor node를 띄우는 김에 `/wheel/odom`이 살아 있으면 그때 함께 재면 된다.
