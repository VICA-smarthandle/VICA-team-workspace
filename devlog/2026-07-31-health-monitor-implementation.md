...
**루프(20)도 프레임(15)도 9.45보다 빠르다.** 즉 루프 주기는 병목이 아니며 `request_hz`를
40으로 올려도 기다리는 시간만 늘어난다.

**약 37 %가 어디서 더 버려지는지는 아직 모른다 `[미규명]`.** 주행 중 CAN이 128 Hz였는데도
9.45로 같았다는 점이 특히 설명되지 않는다. 짐작으로 코드를 고치지 않는다.
