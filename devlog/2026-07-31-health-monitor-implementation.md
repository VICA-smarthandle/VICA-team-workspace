...
#### 수정 — `integration/app-ui-system-monitor` `cb28543`

`select_probe_pid`를 순수 함수로 추가해 셸과 `ros2 run|launch` 런처를 걸러낸다.

**`comm`(실행 파일 이름)으로 좁히는 안은 버렸다.** `comm`은 15자로 잘리고
(`imu_base_link_a`), 무엇보다 motor 프로브의 실제 `comm`이 `keyboard_knob`인데 패턴은
`mdrobot_can_keyboard_knob_node`다 — `comm` 기준으로 바꾸면 **지금 맞는 프로브가 깨진다.**

런처 판정은 argv 원소가 **정확히** `ros2`이거나 `/ros2`로 끝나고 다음 인자가
`run`·`launch`인 경우만 본다. 부분 문자열로 보면 이 저장소의 실제 경로인
`.../vica_ros2_ws/install/...`을 런처로 오인한다(테스트로 고정했다).

**후보가 둘 이상이면 level을 WARN으로 올리고 `candidate_pids`와 함께 문구로 알린다.**
조용히 하나 고르는 것이 이 결함의 본질이었으므로, 모호하면 모호하다고 말하게 했다.
런처만 남으면 pid `None`으로 처리한다 — 런처 CPU를 노드 값으로 보고하지 않는다.

실기 재확인 결과다. 독립 측정(`top`)과 맞는다.

| 프로브 | 수정 전 | 수정 후 | `top` 독립 측정 |
| --- | --- | --- | --- |
| **`imu_adapter`** | **0.0 %** | **43.6 %** | 40.0 / 48.3 / 45.5 % |
| `ekf_node` | 10.6 % | 11.0 % | 11.5 % |
| `motor_node` | 10.8 % | 10.4 % | 11.4 % |
| `controller_server` | 9.4 % | 9.4 % | 10.1 % |

테스트 11건을 추가했다. 패키지 **188건 → 199건, 실패 0**이다.
...
**`/wheel/odom`이 가장 중요한 수정이다.** 근거는 14.14에서 주행 실측으로 정정했다 —
이 절을 쓸 당시의 설명(모터 노드 요청 빈도가 정한다)은 **틀렸다.**
...
- `imu_adapter` 45 %의 최적화는 하지 않았다. **의도적으로 미루었다 — 15절 참조.**

### 14.14 주행 중 `/wheel/odom` 측정 — 14.11의 설명을 정정한다

바퀴를 띄우고 실제로 회전시켜 다시 쟀다. **14.11에 적은 원인 설명은 틀렸다.**

| 국면 | `/wheel/odom` | 바퀴 실측 속도 |
| --- | --- | --- |
| 정지 | 9.45 Hz | 0.000 m/s |
| 일정 속도 | 9.45 Hz | 0.368 m/s |
| 변동 속도 | 9.46 Hz | 0.534 m/s |

CAN TX는 **14.9 → 128.0 Hz(8.6배)**로 늘었는데 **발행 주기는 전혀 변하지 않았다.**
따라서 "모터 노드의 C5 요청 빈도가 정한다"는 14.11의 설명은 반증됐다.

#### 상한을 정하는 것은 `encoder_feedback` 자신이다

```python
self.timer = self.create_timer(1.0 / self.request_hz, self.loop)   # 20.0
def loop(self):
    if not self.read_can_frames():   # 좌·우 둘 다 갱신돼야 True
        return                        # 아니면 발행하지 않는다
```

`encoder.yaml`의 `request_hz`는 "요청 주기"가 아니라 **루프 자체의 주기**이며
`request_position_feedback: false`여도 적용된다. 14.11이 "벤치 전용이라 미적용"이라고 적은
것도 틀렸다.

#### `request_hz`를 올려도 소용없다 — CAN을 직접 읽어 확인했다

```text
C5 오른쪽(motor0)   14.86 Hz
C5 왼쪽(motor1)     14.86 Hz
좌·우 쌍 완성        15.03 Hz    <- 프레임이 오는 속도
루프 주기            20.00 Hz    <- 루프가 도는 속도
/wheel/odom 실측      9.45 Hz    <- 실제 발행
```

**루프(20)도 프레임(15)도 9.45보다 빠르다.** 즉 루프 주기는 병목이 아니다.

#### 규명 완료 — 드라이버가 버스트로 답한다. 버려지는 37 %는 중복이다

`encoder_feedback`의 루프 로직을 **소스 수정 없이 복제**해 돌렸더니 9.40 Hz로 그대로
재현됐다(노드 실측 9.45). 즉 실행 환경이 아니라 **로직과 입력**의 문제다.

CAN 도착 순서를 보니 원인이 드러났다.

```text
간격(ms):  0.0  104.7  0.1  0.2  0.1  108.2  0.1  0.1  0.1  102.9  0.1  104.9
           └ 0.1 ms 간격으로 2~4프레임 ┘   └── 약 105 ms 침묵 ──┘
```

**MDROBOT 드라이버는 C5 응답을 약 105 ms마다 뭉쳐서 보낸다. 버스트 자체가 9.50 Hz이며
이것이 `/wheel/odom` 9.45 Hz와 일치한다.** 쌓인 요청에 같은 값을 여러 번 답하는 것이다.

버스트 안의 값을 비교했다.

```text
버스트 4프레임  오른쪽=[105688, 105688]  왼쪽=[105686, 105686]
버스트 3프레임  오른쪽=[105688, 105688]  왼쪽=[105686]

같은 축 값 비교 — 전부 동일 68건 / 서로 다름 0건
```

**100 % 중복이다.** 0.1 ms면 바퀴가 0.026 mm도 움직이지 않으므로 애초에 다를 수 없다.
따라서 **버려지는 37 %는 정보 손실이 아니고, 9.45 Hz가 드라이버의 진짜 보고 속도다.**

#### `request_hz` 최적값 — 20이 맞다

5~60을 훑었다. 복제본이므로 노드를 건드리지 않았다.

| `request_hz` | 발행 Hz | 폐기 | 평균 지연 | 최대 지연 |
| --- | --- | --- | --- | --- |
| 5 | **4.87** | 67 % | — | — |
| 10 | 9.37 | 38 % | 54.7 ms | 99.6 ms |
| 15 | **9.50** | 37 % | 33.8 ms | 82.1 ms |
| **20 (현재)** | **9.50** | 37 % | **26.6 ms** | **50.5 ms** |
| 30 | 9.50 | 37 % | — | — |
| 40 | 9.50 | 37 % | 12.5 ms | 25.1 ms |
| 60 | 9.37 | 37 % | — | — |

- **발행률은 15부터 평평하다.** 드라이버 보고 속도가 상한이라 올려서 넘을 수 없다.
- **15 미만은 손해다.** 5 Hz에서 4.87로 떨어진다 — 루프가 병목이 된다.
- **지연은 정확히 `주기/2`다.** 0.26 m/s 기준 20 Hz는 0.69 cm, 40 Hz는 0.33 cm 오래된
  값이다. 차이 0.36 cm를 위해 루프를 두 배로 돌릴 이유가 없다.

**`request_hz: 20.0`은 무릎점(15) 위에 여유가 있고 지연도 충분히 짧다. 바꾸지 않는다.**

(지연 측정을 한 번 잘못했다. 처음에는 프레임을 *읽은* 시각을 썼는데 그것은 tick이 도는
순간이라 항상 0에 가깝게 나왔다. 커널 타임스탬프(`msg.timestamp`)로 다시 쟀다.)

#### 좌·우를 따로 발행하는 안은 권하지 않는다

주기는 두 배가 되지만 **회전 시 오차가 들어간다.** `/wheel/odom`은 바퀴 하나의 값이 아니라
로봇의 속도이며 계산에 둘 다 필요하다.

```text
v = (오른쪽 + 왼쪽) / 2          ω = (오른쪽 − 왼쪽) / 축간거리
```

**`ω`는 좌우의 차이**다. 한쪽만 새 값이고 다른 쪽이 66 ms 전 값이면 그 차이에 시간차가
섞인다. 직진에서는 거의 무해하나 회전·가감속에서 `ω`에 오차를 주입한다. **회전 정확도는
14.8에서 이미 문제로 확인된 부분이므로 방향이 반대다.**

#### 속도 환산은 정확하다 — 앞선 우려는 측정 방식 탓이었다

위 표의 "바퀴 실측 속도"는 **최댓값**이라 기동 스파이크에 취약했다. 0.26을 명령했는데
0.534가 찍혀 환산 오류를 의심했으나, **정착 후 평균**으로 다시 재니 정확했다.
...
**환산 계수 문제는 없다.** 바닥 주행 시험이 필요하지 않다.

**다만 표준편차가 크다.** 평균은 정확한데 개별 값이 ±35 % 흔들린다. 위치 차분을 불규칙한
간격으로 나누기 때문으로 보인다. EKF는 공분산으로 이를 다루므로 당장 결함은 아니나
기록해 둔다.

#### 이 절이 보여주는 것 — 주기만 감시하면 안 된다

`probes.yaml`의 토픽 프로브는 전부 **주기만** 본다. 오늘 사례가 그 한계를 보여준다.
...
**"9.45 Hz로 정상 도착하는 값"과 "9.45 Hz로 정상 도착하는 틀린 값"을 지금 구조는 구분하지
못한다.** 값 범위 검사는 초안 8절 범위 밖이며 별도 설계가 필요하다 `[TARGET]`.

## 15. CPU 최적화 대기 목록 `[TARGET]`

**지금은 고치지 않는다.** 전체 기능 시험에서 CPU 부족이 실제로 나타나면 **이 순서대로**
손댄다. before 수치는 아래에 다 있으므로 개선 효과를 바로 잴 수 있다.

### 15.0 지금 급하지 않다고 판단한 근거

2026-08-01 실측이다.

```text
코어 8개 = 최대 800 %
현재 총 사용    354.6 %   (약 44 %, 절반 이상 남음)
load average    4.24 / 8
imu_adapter     46.4 %  =  코어 0.46개  =  전체의 5.8 %
```

**단, 이 값은 유휴 상태다.** Nav2가 비활성이고 음성(STT/TTS)도 실행하지 않았다.
주행과 음성이 동시에 도는 상태를 아직 재지 않았다. **그 측정에서 부족이 나오면 아래를
1순위로 본다.**

### 15.1 1순위 — `imu_base_link_adapter` (before **45 %**)

**낭비인 것은 분명하다. EKF는 이 값을 30 Hz로 쓰는데 200 Hz로 나른다.**
`ekf.yaml`의 `imu0_config`가 쓰는 것은 각속도 z **하나**뿐이다.

비용 구조를 실측으로 나눴다. 같은 200 Hz 토픽을 받아 **카운트만 하는 최소 노드**를 띄워
프레임워크 하한을 쟀다.

| 구간 | CPU | 성격 |
| --- | --- | --- |
| 최소 구독자(콜백에서 카운트만) | **24 %** | rclpy 역직렬화·디스패치. **코드로 못 줄인다** |
| 어댑터 전체 | 43~46 % | |
| 차이 = 우리 코드 | 약 19 %p | TF 조회 + 회전 + 발행 |

**따라서 코드를 완벽하게 고쳐도 24 % 아래로 내려가지 않는다.** 계산 최적화만으로는
45 % → 약 38 %가 한계다.
...
| 방법 | 예상 | 잃는 것 | 비고 |
| --- | --- | --- | --- |
| **A. 카메라 IMU 발행량 200 → 60 Hz** | **45 → 약 14 %** | 데이터 해상도 | Docker `run_d455.sh` 수정 |
| B. 어댑터에서 솎아내기 | 45 → 약 30 % | — | 받는 비용은 그대로라 효과가 작다 |
| C. 정적 TF 캐시 + 공분산 재계산 제거 | 45 → 약 38 % | 없음 | 호스트만 수정 |
| D. C++ 재작성 | 24 % 하한도 사라짐 | 큰 작업 | |

**A가 압도적이다.** 받는 비용 자체가 줄기 때문이다. 60 Hz면 EKF 30 Hz 주기당 2샘플이라
충분하며, `probes.yaml` 주석도 애초에 "400 Hz → 60 Hz로 바뀔 예정"이라고 적어 두었다.

**A를 할 때 함께 고쳐야 하는 것** — `imu_base_link_adapter`의 자이로 편향 보정
표본 수다(`gyro_bias_sample_count`, 기본 4000).

```text
200 Hz:  4000표본 = 20초
60 Hz :  4000표본 = 67초   <- 기동이 너무 느려진다. 1200으로 낮춰야 20초다
```

표본이 줄면 잔차가 커진다(`sigma/sqrt(N)`). 14.8의 표를 다시 만들어 잔차를 확인해야
한다. **이 연결을 놓치면 드리프트가 조용히 되살아난다.**

### 15.2 2순위 — 감시 계층 자신 (before **42.8 %**)

```text
external_diagnostics_node   23.6 %
robot_health_monitor_node   19.2 %
```

**상태를 보려고 만든 도구가 `imu_adapter`와 맞먹는다.** 2026-08-01에 추가한 것이며 아직
아무도 문제 제기를 하지 않았다. 6절이 "감시 도구가 스스로 오탐을 만들 수 있다"고 적었는데
자원 측면의 같은 이야기다.

먼저 볼 곳은 `diagnostic_period_sec: 1.0`과 `process_scan_period_sec: 5.0`,
그리고 토픽 프로브 6개의 구독 비용이다. **`/imu/base_link`를 프로브하지 않는 것이 다행이다**
— 200 Hz 토픽을 하나 더 구독하면 여기도 24 %가 붙는다. 프로브를 추가할 때 이것을 기억한다.
