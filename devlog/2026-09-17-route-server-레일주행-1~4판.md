...
하는 것. 이틀 동안 실주행 14회(bag 14개), 판(版) 7개, 사고 8건, 커밋 9개가 나왔다.
...
- bag: `~/vica_data/bags/route0916_run1·3·5·6·8`, `noroute0916_run4·7`, `route0917_run9·10·11·12·13`, `noroute0917_run14`
...
| | 기존 planner (run7) | **기존 planner (run14, 같은 날 대조)** | **레일 7판 (run13)** |
| --- | --- | --- | --- |
| 완주 | 7/9 (코너 갇힘 1, 손 개입 1) | 14/19 (실패 2·취소 3, **손 개입·초기위치 재설정 2회**) | **15/15, 개입 0** |
| 코너 | 여유 0.14 m 에서 갇힘 | 화장실 출구 코너 115 s·작업실→안내소 66 s 갇힘(벽 여유 0.18~0.30 m) | 화장실 출구 코너 5 s 멈칫, 나머지 통과 |
| 벽에 붙어 가는 비율(여유 <0.45 m) | — | **31.7 %** | **11.2 %** |
| 화장실↔안내소 길 | 휴게실 경유 | 휴게실·방2 복도 경유 13~14 m | 복도 10~11.6 m, 최단 36 s |
| 도착 정렬 | planner 가 미리 맞춤 | 5~19 s, 작업실 −28° | 1~8 s, 15회 전부 ≤12° |

7판 결론: 레일이 무효일 때 "최종 목적지까지 자유주행"하던 구조를 README 방식 2(planner 는 레일 위 3 m 앞
"당근"까지만)로 바꾸자 길 오인이 사라졌다(5판). 그 부작용 둘을 6판이 고쳤고, 6판이 만든 도착 방향 사고를
7판이 고쳐 run13 에서 검증됐다. 같은 날 기존 planner 로 같은 코스를 달린 run14 가 대조군이다. 남은 것은 §7.
...
| run12 | 14:22 | 6판(레일 후퇴·근접 재계획 중단) | 12/12 | 출발점 치명 23, 정렬 5.6~13 s. **방향 사고**: 방2 +79~+96°, 입구 −96°(odom 기준). 기동 후 3분 CPU 과부하 |
| run13 | 15:22 | **7판(레일 원본 분리)** | **15/15** | 도착 yaw 15회 전부 ≤12°, 정렬 1~8 s, 화장실→안내소 36 s 최단. 기동 후 16분 CPU 고부하 상태(레일 무관) |
| run14 | 16:06 | 기존 planner (같은 날 대조) | 14/19 | 화장실 출구 코너 115 s·작업실→안내소 66 s 갇힘 → 실패 2, 손 개입·초기위치 2회. 벽여유 <0.45 m 32 % |
...
### 4.11 run13 — 7판 검증, 그리고 "기동 후 CPU 고부하 상태"

**검증** 도착 순간 로봇 yaw 를 /odom + map→odom 으로 다시 쟀다(/tf 는 끊김 738회라 못 씀). 15회 전부 **≤12°**
(방2 +7/+11, 입구 +3, 작업실 +11, 안내소 −2~−6). 같은 잣대로 run12(6판)는 방2 +79/+96/+80/+82°, 입구 −96° —
사고는 진짜였고 옛 TF 기준 수치(±143°)는 과장이었다. run11(5판) ≤9°. 정렬 시간은 안내소 1.2~7.7 s(run11 13~24 s).
**5판의 정확도와 6판의 속도를 함께 가진 상태**라 되돌릴 이유가 없다. 화장실→안내소 44.7/44.6/**36.0 s**, 휴게실 경유 0.

**출발 좌/우** 3회 전부 CW. 첫 회만 제자리 회전 대신 전진하며 돌아 남쪽으로 0.5 m 밀린 뒤 4 s 좌우 반전 —
휴게실 시도가 아니라 DWB 가 그 순간 고른 궤적 차이(확률적).

**정지 2회** ① 화장실 출구 코너(2.7,4.25) 5 s: 당근 planner 경로가 안쪽으로 파고들어 벽여유 0.35 m·발자국 치명
42칸·정지폴리곤에 벽점. run7 8 s·run11 6+28 s·**run14 115 s** 와 같은 자리. 레일 자체는 가운데(4판은 안 걸림).
② 방2 서쪽 복도(1.45,1.05) 3.4 s: 앞왼쪽 0.32 m 에 라이다 53~61점 → 처음엔 사람으로 봤으나 run13·run14 의
**모든 통과 때** (1.15,0.85) 반경 0.35 m 에 점이 잡힌다(최대 77점) → **지도에 없는 고정물**. 0630 지도 이후 놓인 것.

**CPU 고부하 상태** 주행 중 80 %·load 18.7, 정지 중에도 75 %. 회차별 진단(cpu_percent)으로 보면 ekf 4→33 %,
imu_adapter 5→16 %, controller 17→33 % — nav2 와 함께 기동한 노드 전부 3~8배, 먼저 떠 있던 카메라·모터는 그대로.
run4(기존 planner, 09-16)도 같은 고부하, run7~11·run14 는 정상 → **레일과 무관한 기동 시점 현상**. run12 는 3분 뒤
해소, run13 은 16분 지속 후 주행이 끝나고 해소. 기록기 단독 시험 +0.24코어(원인 아님), 원격 데스크톱 Xorg 0.4 %,
온도 59 °C, nav2 중복 실행(옛 PID 0), /dev/shm 세그먼트 214개 — 원인 미확정. 영향은 컸다: controller 20 Hz 미달
700회(run11 12, run14 19), 액션 ack 타임아웃 19. run12 의 "정체불명 4.3코어"는 ps pcpu(수명 평균) 측정 오류였고
순간 부하 표본기(cpu13.py, 5 % 이상 전수)로 전부 식별됐다.

### 4.12 run14 — 기존 planner 대조 (같은 코스, 순서는 다름)

- **실패 2·손 개입 2**: ① 화장실→방2 출구 코너: planner 경로 자체가 벽여유 0.25 m 로 그려져 (2.17,3.94) 여유 0.18 m
  에서 치명 52~75칸으로 **115 s 정지 → Goal failed** → 사용자가 밀어 (2.0,4.7) 로 옮기고 초기위치 재설정.
  ② 작업실→안내소 남향 통로(−4.35,4.4~4.9): 서쪽 벽에 0.30→0.25 m 로 붙어 치명 40→101칸, **66 s 정지 → 실패** →
  재설정. 레일(run13)은 같은 통로를 x −4.08~−4.17(0.2 m 더 동쪽)로 4회 통과. 둘 다 lattice 가 안쪽 벽으로
  파고드는 코너 문제(base_link 구동륜 이동 뒤 R 0.10 코너 여유 0).
- **길 선택**: 화장실↔안내소를 **휴게실·방2 복도(y≈1)로 13~14 m** 돌아갔다(레일 10~11.6 m). 넓은 바닥이 inflation
  비용이 싸서 lattice 에는 "더 싼 길". 그래서 화장실 출발이 크게 호를 그린다: 첫 경로(앞 4 m)가 y 2.93 까지 내려간다
  (레일 당근은 5.47, 복도선 유지). 당근은 3 m 앞 레일 위라 제자리에서 돌아 복도로 붙는 것이 유일한 싼 길.
- **벽에 붙어 가기**: 주행 중 벽여유 <0.45 m 가 **31.7 %**(레일 11.2 %), 평균 0.77 m(0.92). 방2 서쪽 복도 고정물
  옆에서도 14.7 s 정지(뒤·왼쪽 치명 20~39칸, 레일은 3.4 s).
- 도착: yaw ≤13°(작업실 1회 −28°), 정렬 5~19 s(레일 1~8 s). 출발점 치명 121(레일 27), no valid path 56(9).
- CPU 정상 43~46 %·load 6~8(기동 직후 49 % 한 번). 진단 cpu_percent 는 레일 정상 회차(run8~11)와 동일:
  ekf 3.9/4.0~4.6, imu 4.2/4.2~5.2, controller 17.7/15.8~21.6, 카메라 11.9/12.3~14.5.
  **레일이 더 쓰는 것은 route_server 0.06코어(경로 계산 중앙 1.1 ms) + 전용 lifecycle_manager 뿐**. planner·bt 는
  표본기 기준 둘 다 0.1코어 안쪽.
...
| 구간 | run6 2판 | run7 planner | run8 2판 수리 | run9 3판 | run10 4판 | run11 5판 | run12 6판 | **run13 7판** | **run14 planner** |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| 화장실→안내소 전체 | 38 s | 69 s (사다리) | 42 s | 58 / 68 s | 46 / 70 s | 54 / 46 s | 46 / 43 / 78 s | 45 / 45 / **36 s** | 46 s (휴게실 경유 14 m) |
| 안내소→화장실 | — | — | — | — | — | — | — | 24 / 26 / 26 s | 30 s (13.3 m) |
| 휴게실 경유 틱 | — | — | — | 12 / 12 | 12 / 12 | 0 | 0 | 0 | 14 / 40 |
| 안내소 도착 정렬 | 7 s | — | 9 s | 12 s | 12 / 11 s | 24 / 19 / 13 s | 5.6 / 13 / 8 / 9 s | **1.2 / 4.6 / 5.2 / 7.7 s** | 5.5 / 8.1 / 10.4 s |
| 도착 yaw 오차 | ≤15° | ≤11° | ≤15° | ≤15° | ≤14° | ≤9° | +79~96° (버그) | **≤12°** | ≤13° (작업실 −28°) |
| Starting point in lethal | 8 | 60 | 2 | 4 | 4 | 140 | 23 | 27 | 121 |
| 벽여유 <0.45 m 비율 | — | — | — | — | — | — | — | 11.2 % | 31.7 % |
| 완주 / 손 개입 | 7/7 / 0 | 7/9 / 1 | 7/7 / 0 | 8/8 / 0 | 8/8 / 0 | 9/9 / 0 | 12/12 / 1(장애물 시험) | **15/15 / 0** | 14/19 / 2 |

## 7. 남은 문제와 다음 단계

1. **화장실 출구 코너 파고들기**(당근 planner): 레일 앞 3 m 조각이 비어 있으면 레일을 그대로 따르고, 막혔을 때만
   당근 planner 를 쓰는 안(미승인). 4판이 이 코너에 안 걸린 것이 근거.
2. **막힌 복도는 기다리기**: 당근이 막히면 N초 기다린 뒤에만 목적지 우회. 지금은 틈으로 파고든다(run12 #9).
3. **기동 후 CPU 고부하 상태**: 표본기를 먼저 켜고 nav2 를 띄운 뒤 CPU 50 % 아래를 확인하고 출발. 원인 추적은
   그때 `ros2 node list`·tailscale0 카운터·/dev/shm 목록을 같이 남긴다. 사용자는 원격 접속 2개를 의심.
4. 방2 서쪽 복도 (1.15,0.85) 고정물: 치우거나 지도에 넣는다(0630 지도 이후 놓인 것).
5. 화장실 출발 U턴 방향(확률적): 레일 시작점을 북서에 두거나 당근 최소 거리.
6. global `raytrace_max_range` 3.5→8 (근거 조사 완료, 하나씩). 출발 가속 `velocity_smoother.max_accel[0]` 2.5→0.5 는 레일 안정 뒤.
7. **다음 실주행은 0903 지도(긴 복도)**. 0903_d 레일은 다른 세션이 `--tree` 로 생성(미커밋·실주행 0회). 레일 BT 는
   그래프 모양을 가리지 않지만, 첫 회차는 표본기와 bag 을 걸고 짧게.
