# Route Server(레일 주행) 도입 — 1판에서 7판까지 (2026-09-16 ~ 09-17)
...
하는 것. 이틀 동안 실주행 12회(bag 12개), 판(版) 7개, 사고 8건, 커밋 8개가 나왔다.

- 브랜치: 루트·`vica_ros2_ws` 둘 다 **`test_Route_Server`** (최신 dev + 회전축 수정 d8d3b1a cherry-pick)
- 커밋: ros2 `51213e7`(2판) · `cf73632`(3판) · `ae6e434`(5판) · `69e07f5`(6판) · `0410a56`(7판), 루트 `4eac44f` · `515d249` · `de29b9b`(devlog). 4판은 5판에 흡수
- bag: `~/vica_data/bags/route0916_run1·3·5·6·8`, `noroute0916_run4·7`, `route0917_run9·10·11·12`
...
| | 기존 planner (run7) | 레일 4판 (run10) | **레일 6판 (run12)** |
| --- | --- | --- | --- |
| 완주 | 7/9 (코너 갇힘 1, 손 개입 1) | 8/8 | **12/12** |
| 코너 | 여유 0.14 m 에서 갇힘 | 가운데로, 여유 ≥0.21 m | 같음 |
| 길 오인(휴게실 경유) | — | 46·70 s 배회 | **0** (이동 27~30 s, 역대 최단) |
| 도착 정렬 | planner 가 미리 맞춤 | 최대 31 s | 안내소 5.6~13 s, **그러나 방향 사고**(§4.9) |

7판까지의 결론: 레일이 무효일 때 "최종 목적지까지 자유주행"하던 구조를 README 방식 2
(planner 는 레일 위 3 m 앞 "당근"까지만)로 바꾸자 길 오인이 사라졌다(5판). 그 부작용 둘
(매 틱 planner 의존 → 출발점 치명 정지, 목적지 옆 재계획 → 고리 지나침)을 6판이 고쳤고,
6판이 새로 만든 도착 방향 사고를 7판이 고쳤다(실주행 미검증). 남은 것은 §7.
...
| run11 | 12:55 | 5판(당근·2단계 게이트) | 9/9 | **길 오인 0**, 이동 27~30 s. 부작용: 출발점 치명 140회·정지 11/28 s, 안내소 정렬 24/19 s |
| run12 | 14:22 | 6판(레일 후퇴·근접 재계획 중단) | 12/12 | 출발점 치명 23, 정렬 5.6~13 s. **방향 사고**: 방2 +48~+143°, 입구 −155°. 기동 후 3분 CPU 과부하 |
...
### 4.8 5판 — README 방식 2 "당근" (run11)

**무엇을** `ComputeRoute → 거름망 → TruncatePathLocal(앞 3 m) → GetPoseFromPath(index −1 = 당근)
→ ComputePathToPose(당근)`. planner 가 레일 위 3 m 앞까지만 그리니 뒤에 사람이 있어도 돌아서
레일로 복귀하고 최종 목적지까지 그릴 일이 없다. `IsPathValid` 제거. 2단계는 `GoalReached`
로 게이트. 1판에 포기했던 방식 2 를 기성 노드 둘(`GetPoseFromPath` 설치돼 있으나 미등록)로 만들었다.

**결과** 9/9. 화장실→안내소 **이동 30·27 s(역대 최단)**, 휴게실 경유 0틱, 출발 시 사람을 돌아 나옴.

**부작용 둘**
- 매 틱 planner 가 필요해져 `Starting point in lethal` **140회**(run10 4회). 벽 옆(여유 0.3~0.45 m)에서
  뒤에 사람이 0.5~0.6 m 로 붙으면 발자국 테두리가 치명 칸에 닿아 당근·목적지 둘 다 계획 실패 →
  11 s·28 s 정지, costmap 초기화 60회. 4판까지는 레일 경로가 planner 없이 controller 로 갔기에
  이 상황에서도 움직였다.
- 목적지 0.25 m 안에서 매초 새 경로: lattice 는 제자리 회전을 못 그려 1 m 고리를 내고 DWB 가 쫓아
  지나침(안내소 정렬 24·19 s, yaw −7° 까지 왔다가 +73°).

### 4.9 6판 — 레일 후퇴 + 근접 재계획 중단, 그리고 도착 방향 사고 (run12)

**수리** A `CarrotOrRail = Fallback[ComputePathToPose(당근), ComputeRoute]`(실패 시 레일 그대로),
B `NearGoalKeepPath = Fallback[GoalReached, ComputePathToPose(목적지)]`(0.25 m 안 재계획 중단).

**효과** 출발점 치명 140→23, 벽 옆 정지 0, 0.25 m 안 재계획 46→6~12, 지나침 74°→0°, 안내소
정렬 24→5.6~13 s(180° 제자리 회전 물리 한계 ≈8 s).

**사고** 성공 12회 중 방2 +48/+143/+80/+82°, 입구 −155°, 안내소 −144° 로 **틀린 방향에서 도착
처리**(run11 은 ≤9°). 성공 순간 FollowPath 가 쥔 경로는 전부 **레일 점 1개(방향 identity = 0°)**.
`ComputeRoute` 가 레일을 `{path}` 에 쓰는데, 목적지 옆에서 레일이 점 1개로 줄면 거름망이
거르기 전에 `{path}` 가 그 점으로 덮이고, B 때문에 아무도 덮어쓰지 않아 FollowPath 가 그 점을
목표로 삼았다. 목적지 yaw 가 0° 인 화장실·안내소만 우연히 멀쩡했다. **7판**: 레일 원본을
`{rail_path}` 로 분리, `{path}` 에는 planner 결과와 후퇴용 레일만(`0410a56`, 미검증).

**그 밖에 본 것**
- 화장실 출발 좌/우: 초기 yaw 가 0° 근처면 lattice 가 U턴 방향을 사실상 임의로 고른다. 오른쪽(CW)이면
  남쪽으로 휩쓸려 휴게실 가지 근처(y≈4.5)에서 당근이 1 m 앞이라 ±0.4 제자리 회전 10 s(run12 #3).
  왼쪽이면 일직선. 대책 후보: 레일 시작점을 북서에 두거나 당근 최소 거리.
- 장애물 시험(사람이 1.2 m 복도를 막음): 당근 계획 `no valid path` → 레일 후퇴 → DWB 궤적 없음 →
  목적지 planner 가 틈으로 208점 경로 → 남쪽 벽 쪽으로 틀어(여유 0.30, 앞 치명 27칸) 30 s 정지 →
  손 개입. 기존 planner 도 같은 lattice 라 같은 틈을 찾는다. 막힌 복도는 **기다리는 정책**이 맞다.
- 사람이 뒤에서 떨어져 있어도 영향 없음(A 효과).

### 4.10 CPU·통신 (run12, OS 표본기 2 s + bag)

- 정상 구간(14:25:30~): CPU 47 %, load 8(8코어), route 계산 0.5 ms, DDS 오류 0 → 병목 없음.
- **기동 후 3분(14:22~14:25) 과부하**: CPU 79 %, load 18. `/tf` 끊김 최대 **8.3 s**(0.4 s 초과 147회),
  `/scan` 0.8 s, 액션 ack 타임아웃 8회(`compute_route`·`follow_path`), controller 주기 미달 236회
  (run11 12회). 상위 8개 프로세스 합은 2.1코어라 **나머지 4.3코어의 정체는 미확인**(표본기가 상위
  8개만 기록). 다음엔 5 % 이상 전부 기록.
- 상시 비용: bag 기록기(`ros2`) 0.35코어(시험용), robot_health_monitor 0.20, external_diagnostics
  0.17, mission_manager 0.23 — Python 감시 노드 셋 0.6코어. 다이어트 후보.
...
| 구간 | run6 2판 | run7 planner | run8 2판 수리 | run9 3판 | run10 4판 | run11 5판 | run12 6판 |
| --- | --- | --- | --- | --- | --- | --- | --- |
| 화장실→안내소 전체 | 38 s | 69 s (사다리) | 42 s | 58 / 68 s | 46 / 70 s | 54 / 46 s | 46 / 43 / 78(장애물) s |
| 그중 이동 | 33 s | — | 33 s | — | — | **30 / 27 s** | 40 / 30 s |
| 휴게실 경유 틱 | — | — | — | 12 / 12 | 12 / 12 | **0** | **0** |
| 안내소 도착 정렬 | 7 s | — | 9 s | 12 s | 12 / 11 s | 24 / 19 / 13 s | 5.6 / 13 / 8 / 9 s |
| 도착 yaw 오차(방2·입구) | ≤15° | ≤11° | ≤15° | ≤15° | ≤14° | ≤9° | **+48~143° / −155°** (버그) |
| Starting point in lethal | 8 | 60 | 2 | 4 | 4 | 140 | 23 |
| 완주 | 7/7 | 7/9 | 7/7 | 8/8 | 8/8 | 9/9 | 12/12 |

## 7. 남은 문제와 다음 단계

1. **7판 실주행 검증**: 방2·입구 도착 yaw 오차 ≤14°, 안내소 정렬 ≤10 s, 출발점 치명 한 자릿수.
2. **막힌 복도는 기다리기**: 당근이 막히면 N초 기다린 뒤에만 목적지 우회. 지금은 모든 판이 틈으로 파고든다.
3. 화장실 출발 U턴 방향(확률적): 레일 시작점을 북서에 두거나, 당근 최소 거리(로봇에서 1.5 m 미만이면 더 앞).
4. 기동 후 3분 과부하의 정체: 5 % 이상 프로세스 전수 기록으로 잡는다. Python 감시 노드 3개(0.6코어) 다이어트.
5. global `raytrace_max_range` 3.5→8 (근거 조사 완료, 하나씩).
6. 방2→안내소 고리 우회(19 m): 지름길 엣지. `_route.png` 는 maps/ 밖으로. 안내소→작업실 통로의 지도 밖 물체 확인.
7. 출발 가속(바퀴 3 m/s²): `velocity_smoother.max_accel[0]` 2.5→0.5 — 레일 안정 뒤.
