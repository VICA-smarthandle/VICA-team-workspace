...
### 후속 — `sensor_timeout` 수정으로 EKF 오차를 3.5배 줄였다

위 측정에서 EKF가 거리가 늘수록 과대 보고했다(2 m +0.43 %, 4 m +0.94 %). 원인은
`ekf.yaml`의 `sensor_timeout: 0.1`이었다. **`/wheel/odom`의 실효 주기가 105.8 ms인데
timeout이 100 ms라 매 사이클 timeout을 넘겼다.** robot_localization은 그때 보정을 포기하고
예측만 수행하므로, 가감속 구간에서 직전 속도가 유지되어 거리가 부풀었다.

`0.2`로 고치고(`fix/imu-gyro-bias` `c977011`) 같은 4 m를 다시 쟀다.

| | 1차 (`0.1`) | 2차 (`0.2`) |
| --- | --- | --- |
| 실측 | 4.05 m | 4.055 m |
| **encoder** | 4.0138 m (−0.89 %) | **4.0138 m** (−1.02 %) |
| **EKF** | 4.0880 m (**+0.94 %**) | **4.0443 m (−0.26 %)** |
| EKF 절대 오차 | +3.8 cm | **−1.1 cm** |

**encoder가 소수점 넷째 자리까지 동일하다.** EKF와 무관한 센서이므로 바뀐 것이
`sensor_timeout` 하나뿐임이 증명된다 — 대조군이 성립한다.

그리고 이제 **EKF가 encoder보다 정확하다**(−0.26 % vs −1.02 %). EKF는 encoder에 자이로를
더해 보정하는 구조이므로 원래 그래야 맞으며, **1차에서는 그것이 작동하지 않고 있었다.**

이 값은 드라이버가 정하므로 앞으로도 바뀌지 않는다(14.14절). 다른 센서를 EKF에 추가할 때도
**실효 주기를 먼저 재고 `sensor_timeout`이 그보다 긴지 확인한다.**
