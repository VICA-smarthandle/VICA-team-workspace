...
- 배터리 계측은 **Junctek KG110F 기준으로 작성하되 `[미정]`** (변경 가능), 방향만
  → **2026-08-06 갱신: KG110F 는 스펙 불일치로 기각. 계측부를 직접 제작하고 배터리
  도킹 스테이션을 구현한다. B-1 참조.**
...
### B-1. 계측 하드웨어 — Junctek KG110F `[기각]` (션트 내장 완제품, RS485)

> ## ⚠️ 2026-08-06 갱신 — KG110F 는 쓰지 않는다
>
> **스펙이 맞지 않아 기각됐다**(사용자 판정). 계측부를 **직접 제작**하고, 그것으로
> **배터리 도킹 스테이션을 구현할 예정**이다.
>
> 아래 B-1 이후 내용은 **KG110F 를 전제로 쓰였으므로 그대로 적용되지 않는다.**
> 부품 비교·배선·주문 전 확인 항목은 기각된 후보의 조사 기록으로만 남긴다.
>
> **직접 제작이 아래 표의 부류 2(전류·전압만 읽고 SOC 는 우리가 계산)에 해당하면,
> 문서가 경고한 대로 B-4 를 통째로 다시 써야 한다** — 쿨롱 카운팅 자체 구현,
> 완충–방전 전 사이클 실측, OCV-SOC 테이블, Peukert 계수 추정, 드리프트 리셋 규칙.
> 제작 사양(측정 소자, 션트 정격, 통신 방식, SOC 계산 주체)이 정해지면 B-1·B-4 를
> 재작성한다. `[미정]`
>
> ---
>
...
| 항목 | 확인 방법 |
|---|---|
| **직접 제작 계측부 사양 `[미정]`** | 2026-08-06 에 KG110F 가 스펙 불일치로 기각됐다. 측정 소자·션트 정격·통신 방식·**SOC 계산 주체**를 확정해야 한다. SOC 를 우리가 계산하면(부류 2) **B-4 재작성이 필요하다** |
| 산타리 SLB2418 BMS 통신 유무 | 제조사 문의 031-981-8118 — **계측부 제작보다 먼저.** 있으면 추가 부품 0 이고 직접 제작 자체가 불필요해질 수 있다 |
| 산타리 SLB2418 연속/피크 방전전류·셀 구성·차단 전압 | 같은 문의. 직접 제작 시 션트 정격 산정의 입력이다 |
| ~~KG110F 디스플레이 없이 단독 동작 여부~~ | 기각으로 해소 |
| ~~KG110F 국내 조달처·실제 가격~~ | 기각으로 해소 |
